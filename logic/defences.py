...
async def play_obstacle(game_state, obstacle):
...
async def play_alarm(game_state):
    await play_obstacle(game_state, Supply.ALARM)


async def defend_with_alarm(game_state):
    shelter = game_state.active_player
...
async def play_barricades(game_state):
    await play_obstacle(game_state, Supply.BARRICADES)


async def defend_with_barricades(game_state):
    shelter = game_state.active_player
...
async def play_mine_field(game_state):
    await play_obstacle(game_state, Supply.MINE_FILED)


async def defend_with_mine_field(game_state):
    shelter = game_state.active_player

    async def use_mine_field(zombie):
